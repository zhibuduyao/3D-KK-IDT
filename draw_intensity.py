...
def create_image():
    print("正在创建样本...")
    z_coords = torch.arange(sp.nz, device=sp.device, dtype=torch.float32)
    ...
    dist_sq = (z_grid - sp.nz//2)**2 + (x_grid - sp.nx//2)**2 + (y_grid - sp.ny//2)**2
    n_sample = torch.full((sp.nz, sp.nx, sp.ny), sp.n_medium, device=sp.device)
    n_sample[dist_sq < (sp.nz//4)**2] = 1.34
    dist_sq = (z_grid - sp.nz//2)**2 + (x_grid - sp.nx//4)**2 + (y_grid - sp.ny//4)**2
    n_sample[dist_sq < (sp.nz//4)**2] = 1.34
    ...
    for i in tqdm.tqdm(range(N_ill)):
        for j in range(sp.nz):
            ...
            model1=wpm.WavePropagationMethod(623,sp.NA_obj,1.33,1.33,i*360/N_ill,53,sp.objx_pixel_size,sp.objz_pixel_size,20,sp.device)
            model2=fw.BeamPropagationMethod(623,sp.NA_obj,1.33,1.33,i*360/N_ill,53,sp.objx_pixel_size,sp.objy_pixel_size,20,sp.device)
            intensity=model1.forward(n_sample,defocus_distance)
            
        
            # torch.save(intensity,f'./data/intensity/{i}.pt')
            intensity_np = intensity.cpu().numpy()
            tifffile.imwrite(f'{save_dir}/xdeg{i*360/N_ill:.1f}_zdeg53.0_z{defocus_distance}.tiff', intensity_np)
